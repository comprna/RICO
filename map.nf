...
params.rDNAgene_bed = "/g/data/xc17/zaka/res/human/rDNAx5/rDNAgene.bed"
params.SCG_bed      = "/g/data/xc17/zaka/res/human/SCG/SCG-2.bed"
...
process align {
  ...
  publishDir "${params.results_dir}", mode: 'symlink', overwrite: true
  ...
  script:
    ...
    ${params.samtools} fastq "${unalign_bam}" -T "MM,ML" \
      ...
      | ${params.samtools} sort -@ ${task.cpus} -m 2G -o "${out_bam}"
...
process mean_cn {
  ...
  script:
    ...
    def out_scg  = "${bamBase}_SCG-2_perBase.coverage"
   ...
   echo "[\$(ts)] [mean_cn] CN=\${CN}" 1>&2
    ...
    ############################ 5) Summary record
    {
      ...
      echo -e "${bamBase}.bam\\t${params.rDNA_contig}\\t${params.rDNAgene_bed}\\t${params.SCG_bed}\\t\${RDNA_SUM}\\t\${SCG_MEAN}\\t\${CN}"
    } > "${out_sum}"
